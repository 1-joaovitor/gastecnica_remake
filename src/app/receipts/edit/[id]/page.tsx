...
import { getReceiptById, updateReceipt, validateReceiptSignature } from '@/services/receipt';
...
export default function EditReceiptPage() {
  ...
  const loadReceipt = async () => {
    try {
      ...
      // Preencher o formulário com os dados do recibo
      ...
      setValue('clientState', data.clientState || '');
      setValue('clientZipCode', data.clientZipCode || '');
      setValue('description', data.description);
      setValue('amount', data.amount);
      setValue('status', data.status);
      setValue('digitalSignature', data.digitalSignature || '');
      // Informações da empresa são fixas no PDF
      setValue('receiptDate', data.receiptDate ? new Date(data.receiptDate) : undefined);
      setValue('dueDate', data.dueDate ? new Date(data.dueDate) : undefined);

      if (data.client) {
        setValue('client', { id: data.client.id });
      }
    } catch (error) {
  ...
  const onSubmit = async (data: ReceiptFormData) => {
    try {
      ...
      await updateReceipt(params.id as string, data);
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Editar Recibo</h1>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex justify-end space-x-4">
          ...
          >
            {loading ? 'Atualizando...' : 'Atualizar Recibo'}
          </button>
        </div>
      </form>
    </div>
